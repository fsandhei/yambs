...
impl Dependency {
    ...
    ) -> Result<DependencyNode, error::DependencyError> {
    ...
    fn detect_dependency(
        &self,
        dep_registry: &mut DependencyRegistry,
    ) -> Result<Vec<DependencyNode>, error::DependencyError> {
    ...
    pub fn read_and_add_mmk_data(
        self: &mut Self,
    ) -> Result<mmk_parser::Mmk, error::DependencyError> {
    ...
    fn detect_cycle_from_dependency(
        ...
        if dependency.borrow().is_in_process() {
            if dependency.borrow().is_in_process() {
                return Err(error::DependencyError::Circulation(
