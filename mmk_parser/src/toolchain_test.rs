...
fn parse_compiler_test() {
    ...
    assert!(result.is_ok());
    assert_eq!(
        toolchain.config[&Constant::new("compiler")],
        PathBuf::from("/some/path/to/gcc")
    );
}
...
fn parse_linker_test() {
    ...
    assert!(result.is_ok());
    assert_eq!(
        toolchain.config[&Constant::new("linker")],
        PathBuf::from("/some/path/to/ld_gold")
    );
}
